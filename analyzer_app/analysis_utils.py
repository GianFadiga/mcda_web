...
#     except Exception as e:
#         print(f"\nOcorreu um erro inesperado ao analisar {file_path}: {str(e)}")


# if __name__ == "__main__":
#     import sys
#     if len(sys.argv) > 1:
#         analyze_electronics_data(sys.argv[1])
#     else:
...
class DataAnalyzer:
    ...
    def _load_raw_data(self) -> None:
        """Carrega os dados brutos do arquivo CSV, detectando o separador."""
        try:
            with open(self.file_path, 'r', encoding='UTF-8', newline='') as f:
                ...
                if header.count(';') > header.count(','):
                    separator = ';'
                    print("Separador ';' detectado (padrão Excel).")
                else:
                    separator = ','
                    print("Separador ',' detectado (padrão comum).")

            # Carrega o DataFrame com o separador detectado
            self.df = pd.read_csv(
                self.file_path,
                encoding='UTF-8',
                sep=separator,
    ...
    def _extract_configurations(self) -> None:
        ...
        # Renomeia a primeira coluna temporariamente para um nome previsível
        first_col_name = self.df.columns[0]
        self.df.rename(columns={first_col_name: 'ID'}, inplace=True)

        # Extrai as configurações baseando-se nos novos identificadores na coluna 'ID'
        self.weights = self.df[self.df['ID'] == 'PESO'].iloc[0].drop('ID').dropna().astype(float)
        self.data_types = self.df[self.df['ID'] == 'TIPO'].iloc[0].drop('ID').dropna()
        self.proportionality = self.df[self.df['ID'] == 'FUNCAO'].iloc[0].drop('ID').dropna()
        
        # Restaura o nome original da coluna
        self.df.rename(columns={'ID': first_col_name}, inplace=True)
        self.model_column = first_col_name
    ...
    def _prepare_calculation_data(self) -> None:
        """Prepara o DataFrame para cálculos, removendo linhas de configuração."""
        # Filtra para manter apenas as linhas que não são de configuração
    ...
    def _convert_numeric_column(self, col: str) -> None:
        """Converte coluna numérica e valores de referência."""
        # Garante que os valores de referência para conversão existam
        if col in self.good_values:
            self.good_values[col] = pd.to_numeric(self.good_values[col], errors='coerce')
        if col in self.neutral_values:
            self.neutral_values[col] = pd.to_numeric(self.neutral_values[col], errors='coerce')
        
        # Converte a coluna de dados principal
        self.calculation_df[col] = pd.to_numeric(self.calculation_df[col], errors='coerce')
    ...
    def _convert_boolean_column(self, col: str) -> None:
        ...
        # Garante que os valores de referência para conversão existam
        if col in self.good_values:
            self.good_values[col] = str(self.good_values[col]).strip().upper() == 'TRUE'
        if col in self.neutral_values:
            self.neutral_values[col] = str(self.neutral_values[col]).strip().upper() == 'TRUE'
    ...
    def _calculate_string_score_value(self, value: str, column: str) -> float:
        ...
        try:
            # Identifica as linhas de dados válidas para o mapeamento
            config_identifiers = ['PESO', 'TIPO', 'FUNCAO', 'BOM', 'NEUTRO']
    ...
    def _classify_numeric_value(
        ...
        """Classifica um valor numérico como Positivo, Negativo ou Neutro."""
        if pd.isna(neutral_value) or pd.isna(value):
            return 'Neutra'
